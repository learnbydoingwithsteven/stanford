# Reference: https://stanford-cs324.github.io/winter2022/lectures/capabilities
...
- standard NLP benchmarks (e.g., question answering), as well asquirky one-off demos (e.g., using a new word in a sentence).
...
- On some tasks such as language modeling, GPT-3 exceeds the state-of-the-art by a huge margin.On others, where GPT-3 is competing against systems that are trained with large amounts of labeled data, it lags far behind.
...
- Adaptation: How do we reduce the task to language modeling (via prompting)?Results: What are the quantitative numbers compared to task-specific state-of-the-art models?
...
- the full GPT-3 model (davinci), which has 175 billion parametersusing in-context learning with as many training instances as you can stuff into the prompt.
...
- Language modelingQuestion answeringTranslationArithmeticNews article generationNovel tasks
- Question answeringTranslationArithmeticNews article generationNovel tasks
- TranslationArithmeticNews article generationNovel tasks
- ArithmeticNews article generationNovel tasks
- News article generationNovel tasks
- Novel tasks
The goals of this lecture is to provide:

- an overview of tasks in NLP (independent of large language models),a sense of how well GPT-3 works, anda taste for the art of prompt engineering.
- a sense of how well GPT-3 works, anda taste for the art of prompt engineering.
...
- Problem: language model doesn’t know it should be producing the final word of the sentence.Solution: frame it more explicitly as a input-output mapping and use in-context learning with additional examples (demo):
...
- taste them as they place them on plates.put the frosting on the cake as they pan it.come out and begin decorating the cake as well.
- put the frosting on the cake as they pan it.come out and begin decorating the cake as well.
...
- Length-normalized probability: \(\text{score}(x, y) = \frac{p(x, y)}{\text{num-tokens}(y)}\). This fixes the length bias. However, given two answers of the same length, the model still might prefer the more popular entity.Frequency-normalized probability: \(\text{score}(x, y) = \frac{p(y \mid x)}{p(y \mid x_0)}\), where \(x_0\) is a neutral string like \(\nl{Answer:}\). This lowers the score for answers that happen to just be common (e.g., \nl{John}). Compare demo versus demo.
...
- The standard evaluation dataset is the WMT’14 and WMT’16 datasets.Since there are multiple possible translations, the (automatic) evaluation metric is BLEU (which captures a notion of n-gram overlap).
...
- This presents a lower bound on how well one can do in machine translation; you would definitely want to leverage the large amount of parallel corpora (aligned input-output pairs).Results from French and Romanian are similar.Results from English to a foreign language is much worse, which is expected since GPT-3 is primarily an English language model.
- Results from French and Romanian are similar.Results from English to a foreign language is much worse, which is expected since GPT-3 is primarily an English language model.
...
- Dataset: title/subtitles taken from newser.comEvaluation: humans rated articles based on how likely the article was likely to be written by a machine
...
- Massive Multitask Language Understanding: 57 multiple-choice problems spanning mathematics, US history, computer science, law, etc.TruthfulQA: question answering dataset that humans would answer falsely due to misconceptions.
...
- GPT-3 can perform extremely well or be very medicore.Both increasing the size of the model and the number of examples helps performance.There are a few heuristic ways of adapting the language model to the task of interest.Why does this work? No one knows.
- Both increasing the size of the model and the number of examples helps performance.There are a few heuristic ways of adapting the language model to the task of interest.Why does this work? No one knows.
- There are a few heuristic ways of adapting the language model to the task of interest.Why does this work? No one knows.
